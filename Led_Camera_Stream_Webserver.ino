// Dual-Server ESP32-CAM Sketch: Stream & Command Servers
// --------------------------------------------------------
// Two independent HTTP servers: one for video streaming (port 80)
// and one for receiving LED commands (port 81), each with its own
// control socket to avoid blocking or port collisions.
...
// ——— HTTP server handles ———
httpd_handle_t stream_httpd = NULL;  // for video streaming on port 80
httpd_handle_t cmd_httpd    = NULL;  // for LED commands on port 81

// ——— Single-page HTML (served on port 80) ———
...
void start_stream_server() {
  ...
  cfg.server_port      = 80;
  cfg.ctrl_port        = 32768;           // unique control socket
  cfg.lru_purge_enable = true;

  if (httpd_start(&stream_httpd, &cfg) == ESP_OK) {
    httpd_uri_t home = { "/",       HTTP_GET, index_handler,    NULL };
    httpd_uri_t mjpg = { "/stream", HTTP_GET, stream_handler,   NULL };
    httpd_register_uri_handler(stream_httpd, &home);
...
void start_cmd_server() {
  httpd_config_t cfg = HTTPD_DEFAULT_CONFIG();
  cfg.server_port = 81;
  cfg.ctrl_port   = 32769;  // different from stream's ctrl_port
...
void setup() {
  ...
  // Camera init — ordering must match esp_camera.h
...
void loop() {
  // Servers run in background tasks; nothing here
}
